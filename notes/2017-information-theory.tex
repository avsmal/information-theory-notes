...
Последовательность изложения материала данного курса основана на классической статье Колмогорова ,,Три подхода к определению понятия количества информации`` (1965).
...
\begin{example}
Предположим, что мы хотим узнать некоторое неизвестное упорядочение множества $\{1,2,3,4,5\}$. Нам стало известно, что \(1>2\) или \(3>4\). Сколько битов информации мы узнали? Множество \(A\) состоит из \(5!\) перестановок, 
множество \(B\)~--- из перестановок, которые удовлетворяют новому условию. Легко проверить, что \(|B| = 90\). Итого мы узнали \(\log 120 - \log 90 = \log(4/3)\) битов.
\end{example}
...
\[\chi(A) \le \chi_{12}(A) + \chi_{13}(A) + \chi_{23}(A).\]
\end{theorem}
\begin{corollary}
...
Верхняя оценка \(\lceil\log N\rceil \) достигается в обоих случаях, если задавать вопросы про биты двоичного представления загаданного числа.
...
\item \( \chi_A(S) = \chi(A) \le \chi(S) \le \chi_Q(S) + \chi_{Q|A}(S) \le k + 0 = k. \)
\end{itemize}
Таким образом получаем, что \(\log N \le \chi(A) \le k\).
...
где \(\alpha^2 = 1 - \alpha\), \(\alpha=(\sqrt 5 - 1) / 2\). При любом ответе мы заплатим \(c = 1/(-\log \alpha)\approx 1.44\) рублей за бит, а в целом~— \(\log n / (-\log\alpha)\) рублей.
...
\paragraph{Верхняя оценка.} Будет сортировать вставкой с бинарным поиском места вставки. Количество сравнений:
\[
...
Всего исходов \(2\cdot 13 + 1 > 27\), но определить тем не менее нельзя. Аппарата информации по Хартли недостаточно.
...
\item \(H(\alpha) \le \log k\), причём \(H(\alpha) = \log k\) \(\iff\) \(\alpha\) распределена равномерно.
...
\]
\end{proof}
Пусть \(\alpha\) и \(\beta\)~--- случайные величины. Энтропию их совместного распределения будем обозначать \(H(\alpha,\beta)\).
\begin{lemma}
...
\[
\sum_{i,j}p_{i,j} \log\frac1{p_{i,j}} \le
\sum_{i}\sum_{j} p_{i,j} \log\frac1{p_{i,*}} +
\sum_{j}\sum_{i} p_{i,j} \log\frac1{p_{*,j}}.
\]
Перенесём всё в левую часть и применим неравенство Йенсена.
\[
\sum_{i,j}p_{i,j} \log\frac{p_{i,*}p_{*,j}}{p_{i,j}} \le
\log\sum_{i,j}p_{i,j} \frac{p_{i,*}p_{*,j}}{p_{i,j}} =
\log\sum_{i,j}(p_{i,*}p_{*,j}) = 
\log \underbrace{\left(\sum_{i}p_{i,*}\right)}_1\cdot
     \underbrace{\left(\sum_{j}p_{*,j}\right)}_1 = 0.
\]
Равенство в неравенстве Йенсена достигается, если все точки равны, т.е. 
для любых \(i,j\) \(\frac{p_{i,*}p_{*,j}}{p_{i,j}} = c\) для некоторой константы \(c\). Несложно заметить, что \(c = 1\), т.к. выполняется следующее равенство \(\sum_{i,j} {p_{i,*}p_{*,j}} = c \sum_{i,j}{p_{i,j}}\) в котором обе суммы равны 1. Таким образом \(\alpha\) и \(\beta\) независимы.
...
\end{proof}
\begin{corollary} \(H(\alpha,\beta) \le H(\alpha)\), причём равенство достигается тогда и только тогда, когда \(\beta = f(\alpha) \).
\end{corollary}
\begin{proof}
...
\begin{theorem}[Неравенство Макмилана]\label{thm:mcmill}
...
\begin{theorem}[Неравенство Крафта]
    Для префиксных кодов верно неравенство Макмиллана.
\end{theorem}
\begin{proof}
...
\begin{statement}
    ...
    \(\{\seqn{\ell}{n}\}\),
    удовлетворяющие неравенству Крафта
    \[
...
\begin{proof}[Доказательства теоремы~\ref{thm:mcmill}]
    ...
    Предположим теперь, что неравенство Крафта не выполняется, т.e.
    \[
...
\end{lemma}
\begin{corollary}
    Для кода Хаффмана выполняется теорема Шеннона \ref{thm:shannon:optcode}.
\end{corollary}
\begin{remark}
...
\begin{proof}
    ...
    \[
    \mathrm {P}[|X-\mu |\ge \delta]\le \frac {\sigma^2}{\delta^2} =
    \frac{np_i(1-p_i)}{\delta^2}. 
    \]
\end{proof}
...
\begin{proof}
    ...
    произвольного $\delta$-типичного слова с $n_i = np_i + \Delta_i$,
    где $\Delta_i \le \delta n$. Тогда~\eqref{eq:typicalwords} изменится следующим образом:
    \[    
        \dots = \sum_{i=1}^k np_i\cdot
...
\begin{proof}
    ...
    \begin{tikzpicture}
        ...
        \node[circ, label=below:{$S_0\mid A_{i+1}\cup B_i\mid A_{i+1}\cup B_i$}] (c) at (1.5,0) {}; 
        \node at (0.9,0.4) {$0$};
        \node at (1.5,1.3) {$\ge0$};
        \node at (2.1,0.4) {$0$};
...
\begin{itemize}
    ...
    \item Аналогично неравенству Крафта для префиксных кодов.
\end{itemize}
\end{proof}
